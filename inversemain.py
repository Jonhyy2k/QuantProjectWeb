...
def calculate_sigma(data):
    ...
    try:
        ...
        # INVERT THE SIGNAL - Fix for inverted performance
        original_sigma = final_sigma
        final_sigma = 1 - final_sigma
        print(f"[INFO] SIGNAL INVERTED: Original sigma={original_sigma:.3f}, Inverted sigma={final_sigma:.3f}")
...
def build_lstm_model(input_shape):
    ...
    --------
    keras.Model
        Compiled LSTM model with advanced architecture
    """
    try:
        # Advanced architecture parameters (fixed)
        base_units = 1024
        lstm_layers = 6
        dense_layers = 5
        dropout_rate = 0.3
        ...
        for i in range(1, lstm_layers - 1):  # Notice we go up to lstm_layers-1 (save last layer for later)
            ...
            if i % 2 == 0:
                ...
                if x.shape[-1] != first_layer_output.shape[-1]:
                    # Use 1x1 convolution for dimension matching
                    res_projection = Conv1D(x.shape[-1], 1)(first_layer_output)
                    x = tensorflow.keras.layers.add([x, res_projection])
                else:
        ...
        for i in range(dense_layers):
            dense_units = dense_unit_base // (2 ** i)
