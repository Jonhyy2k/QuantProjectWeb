...
def main(user_id, symbol):
    ...
    try:
        ...
        ts = TimeSeries(key='IE4EDZ78E3WEUZKO', output_format='pandas')
        data, meta_data = ts.get_daily(symbol=symbol, outputsize='full')
        logging.info(f"Retrieved {len(data)} days of data for {symbol}")

        # Step 2: Perform analysis (simplified example)
        current_price = float(data['4. close'].iloc[-1])
        sigma_score = np.std(data['4. close'].pct_change()) * np.sqrt(252)  # Annualized volatility
        price_target_30d = current_price * (1 - 0.012)  # -1.2% as per your log
        price_target_60d = current_price * (1 - 0.025)  # -2.5% as per your log
        ...
        result = {
            ...
            'price': current_price,
            'sigma': sigma_score,
            'recommendation': 'Hold' if sigma_score < 0.3 else 'Sell',
            'predictions': {
                'price_target_30d': price_target_30d,
                'expected_return_30d': -1.2,
                'price_target_60d': price_target_60d,
                'expected_return_60d': -2.5,
                'historical_dates': data.index,  # This will be handled by append_stock_result
                'historical_prices': data['4. close'].tolist(),
            },
            'risk_metrics': {
                'max_drawdown': -0.15,
                'sharpe': 1.2,
            ...
            'plot_path': f"prediction_plots/{symbol}_prediction_{datetime.now().strftime('%Y%m%d_%H%M%S')}.png"
        }

        # Step 4: Save the result
        append_stock_result(user_id, symbol, result)

        # Step 5: Fetch the new analysis ID
        conn = sqlite3.connect(DB_FILE)
        cursor = conn.cursor()
        cursor.execute('''
            SELECT id
        ...
        analysis_id = row[0]  # Fix: Use row[0] instead of row['id']
        conn.close()
